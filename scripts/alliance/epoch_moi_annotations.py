...
def process_moi_session(
    ...
    try:
        ...
        logger.info(f"Session duration: {metadata.get('Duration', 0):.1f}s ({metadata.get('Duration', 0)/60:.1f}min)")
        ...
        for method in ['fixed', 'nsplit', 'sliding']:
            ...
            if col in df_epoched.columns:
                ...
                all_epochs = [eid for epoch_list in df_epoched[col] for eid in epoch_list]
                if all_epochs:
                    min_epoch = min(all_epochs)
                    max_epoch = max(all_epochs)
                    n_unique_epochs = len(set(all_epochs))
                    logger.info(f"  {method}: {n_unique_epochs} unique epochs (range: {min_epoch}-{max_epoch})")
                else:
                    logger.info(f"  {method}: No epochs assigned")
        
        if dry_run:
            logger.info("[DRY RUN] Would save epoched file")
            logger.info(f"  Preview of epoched data:")
            # Show first few rows with a sample of data
            preview_cols = ['start', 'end', 'start_seconds', 'end_seconds', 'epoch_fixed', 'epoch_nsplit', 'epoch_sliding']
            logger.info(f"\n{df_epoched[preview_cols].head(10)}")
        else:
            # Save epoched file
            output_file = writer.save_epoched_moi(df_epoched, metadata, group_id, session_id)
        ...
        logger.error(f"Error processing {group_id}/ses-{session_id}: {e}", exc_info=True)
        return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Epoch MOI annotations"
    )
    parser.add_argument(
        '--group', '-g',
        type=str,

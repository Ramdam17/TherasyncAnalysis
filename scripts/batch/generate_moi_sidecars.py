...
def get_session_duration(family_id: str, session_id: str, data_root: Path) -> Optional[float]:
    """
    ...
    bvp_file = (
        data_root / f"sub-{subject_id}" / f"ses-{session_id}" / "physio" /
        f"sub-{subject_id}_ses-{session_id}_task-therapy_recording-bvp.tsv"
    )
    
    if not bvp_file.exists():
        logger.warning(f"BVP file not found for {subject_id}/ses-{session_id}: {bvp_file}")
        return None
    ...
    try:
        ...
        logger.info(f"Found duration for {family_id}/ses-{session_id}: {duration:.2f}s ({duration/60:.1f}min)")
        return duration
...
def process_moi_files(data_root: Path, dry_run: bool = False) -> Dict[str, int]:
    ...
    moi_files = list(sourcedata_dir.glob("*/*/moi_tables/*_desc-alliance_annotations.tsv"))
    ...
    for tsv_file in sorted(moi_files):
        ...
        if duration is None:
            logger.error(f"  Could not determine duration, skipping")
            stats['errors'] += 1
            continue

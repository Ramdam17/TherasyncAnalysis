...
def create_video_from_frames(
    ...
    if format == "mp4":
        ...
        cmd = [
            ...
            "-i", frames_pattern,
            "-i", str(palette_path),
            "-lavfi", f"fps={fps},scale=800:-1:flags=lanczos[x];[x][1:v]paletteuse",
            str(output_path)
        ]
    else:
    ...
    try:
        ...
        logger.info(f"Created video: {output_path} ({output_path.stat().st_size / 1024 / 1024:.1f} MB)")
        return True
...
def main():
    ...
    parser.add_argument(
        ...
        help="Epoching method"
    )
    parser.add_argument(
        "--dyads",
        nargs="+",
        help="Specific dyads to process (default: all)"
    )
    parser.add_argument(
        "--max-epochs",
        type=int,
        help="Limit epochs per dyad (for testing)"
    )
    parser.add_argument(
        "--video",
        action="store_true",
        help="Also create videos from frames"
    )
    parser.add_argument(
        "--video-format",
        choices=["mp4", "gif"],
    ...
    for i, dyad in enumerate(dyads):
        ...
        if generate_frames_for_dyad(
            ...
            if args.video and frames_dir.exists():
                ...
                if create_video_from_frames(frames_dir, video_path, args.fps, args.video_format):
                    success_videos += 1
        else:
            failed.append(dyad)

...
def compute_inter_session(
    ...
    for i, ((subj1, ses1), (subj2, ses2)) in enumerate(pairs, 1):
        ...
        try:
            ...
            logger.error(f"Failed to compute ICD for {subj1}/{ses1} vs {subj2}/{ses2}: {e}")
            failed += 1
...
def compute_intra_family(
    ...
    # Get dyad pairs and method
    pairs = dyad_loader.get_intra_family_pairs(family=family, session=session, task=task)
...
Examples:
  ...
  python scripts/physio/dppa/compute_dppa.py --mode both --task all --batch
    ...
    parser.add_argument('--mode', type=str, required=True, choices=['inter', 'intra', 'both'],
    ...
    for task in tasks:
        ...
        if args.mode == 'inter' or args.mode == 'both':
            ...
            stats = compute_inter_session(
                loader, dyad_loader, calculator, writer,
                task=task, dry_run=args.dry_run
            )
            logger.info(f"Inter-session stats: {stats['processed']}/{stats['pairs']} pairs, {stats['failed']} failed")
        ...
        if args.mode == 'intra' or args.mode == 'both':
            ...
            stats = compute_intra_family(
                loader, dyad_loader, calculator, writer,
                task=task, family=args.family, session=args.session,
                dry_run=args.dry_run
            )
            logger.info(f"Intra-family stats: {stats['processed']}/{stats['pairs']} pairs, {stats['failed']} failed")
            for key in total_stats:
                total_stats[key] += stats[key]

...
def build_dyads_config(
    ...
    for family_id, group in participants.groupby('family_id'):
        ...
        for session in sorted(all_sessions):
            present = [m for m in family_members 
            ...
            if len(present) >= 2:  # Need at least 2 for a dyad
                ...
                for p1, p2 in combinations(present, 2):
                    # Get roles
                    role1 = participants[participants['participant_id'] == p1]['role'].values[0]
                    role2 = participants[participants['participant_id'] == p2]['role'].values[0]
...
def print_statistics(config: dict):
    ...
    n_therapist_patient = sum(1 for d in real_dyads if d['dyad_type'] == 'therapist-patient')
    n_patient_patient = sum(1 for d in real_dyads if d['dyad_type'] == 'patient-patient')
    ...
    for family_id in sorted(config['families'].keys()):
        ...
        print(f"  {family_id}: {len(family_dyads)} dyads across {len(sessions)} sessions")
...
Examples:
  ...
  python scripts/physio/dppa/generate_dyads_config.py -v
    ...
    parser.add_argument(
        '--participants', type=str, default='data/raw/participants.tsv',
        help='Path to participants.tsv file'
    )
    parser.add_argument(
        '--derivatives', type=str, default='data/derivatives/preprocessing',
        help='Path to preprocessing derivatives directory'
    )
    parser.add_argument(
        '--output', '-o', type=str, default='config/dppa_dyads_real.yaml',
        help='Output YAML file path'
    )
    parser.add_argument(
        '--intra-method', type=str, default='sliding_duration30s_step5s',

...
def create_frame(
    ...
    if all_rr:
        ...
        ax_poincare.plot([rr_min, rr_max], [rr_min, rr_max], 'k--', alpha=0.3, linewidth=0.5)
    
    ax_poincare.grid(True, alpha=0.2)
    ax_poincare.legend(loc='upper left', fontsize=9)
    
    # === TOP-RIGHT: ICD Evolution ===
    ax_icd.set_title(f"ICD Evolution", fontsize=12, fontweight='bold')
    ...
    ax_icd.plot(times, icd_values, color=colors.get('therapy', '#d62728'), 
                linewidth=2, alpha=0.8)
    ax_icd.scatter([time_min], [icd_values[-1]], color='red', s=80, zorder=10, 
                   edgecolors='darkred', linewidths=2)
    ax_icd.grid(True, alpha=0.2)
    
    # === BOTTOM-LEFT: Centroid Averages ===
    ax_centroids.set_title(f"Centroid Averages ((X+Y)/2)", fontsize=12, fontweight='bold')
    ...
    ax_ratios.set_title(f"SD1/SD2 Ratios", fontsize=12, fontweight='bold')
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = project_root / 'data' / 'derivatives' / 'dppa' / 'frames' / args.dyad
    ...
    try:
        ...
        logger.info(f"Loaded RR intervals: {len(rr_df_1)} ({subject1}), {len(rr_df_2)} ({subject2})")
    except Exception as e:
    ...
    for epoch_id in tqdm(range(max_epochs), desc="Generating frames"):
        try:
            ...
            poincare_data_1 = animator.compute_poincare_for_epoch(rr_df_1, epoch_id, args.method)
            poincare_data_2 = animator.compute_poincare_for_epoch(rr_df_2, epoch_id, args.method)
            ...
            cumulative_data['centroid_avg_1'].append((centroid_1['centroid_x'] + centroid_1['centroid_y']) / 2)
            cumulative_data['centroid_avg_2'].append((centroid_2['centroid_x'] + centroid_2['centroid_y']) / 2)

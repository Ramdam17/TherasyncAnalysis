...
def find_icd_file(mode: str, task: str, dppa_dir: Path) -> Path:
    ...
    if not matches:
        raise FileNotFoundError(f"No ICD file found for mode={mode}, task={task} in {subdir}")
...
def process_task(
    ...
    try:
        ...
        if stats.get('pseudo') is not None:
            ...
            logger.info(f"Pseudo dyads: n={meta['n_pseudo_dyads']}, "
            ...
            logger.info(f"Mode: Intra-family (real dyads only)")
            logger.info(f"Real dyads: n={meta['n_real_dyads']}")
...
Output:
  Figures are saved to data/derivatives/dppa/figures/{mode}/{task}/
    ...
    parser.add_argument(
        '--all',
        action='store_true',
        help='Process all modes and tasks'
    )

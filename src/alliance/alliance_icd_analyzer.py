...
class AllianceICDAnalyzer:
    ...
    def compute_descriptive_stats(
        ...
        stats_df = data.groupby(group_by)['icd'].agg([
            ('mean', 'mean'),
            ('std', 'std'),
            ('median', 'median'),
    ...
    def compute_session_level_stats(
        ...
        dyad_stats = data.groupby(['dyad', 'dyad_type', 'family', 'session', 'alliance_label']).agg({
            'icd': ['mean', 'std', 'count']
        }).reset_index()
        
        dyad_stats.columns = ['dyad', 'dyad_type', 'family', 'session', 
                              'alliance_label', 'mean_icd', 'std_icd', 'n_epochs']
        
        # Then aggregate across dyads
        session_stats = dyad_stats.groupby(['alliance_label', 'dyad_type']).agg({
            'mean_icd': ['mean', 'std'],
            'n_epochs': 'sum',
            'dyad': 'count'
        }).round(4)
        
        session_stats.columns = ['grand_mean_icd', 'between_dyad_std', 
                                  'total_epochs', 'n_dyads']
    ...
    def generate_report(
        ...
        lines.append(f"Real dyad observations: {len(data[data['dyad_type'] == 'real'])}")
        lines.append(f"Pseudo-dyad observations: {len(data[data['dyad_type'] == 'pseudo'])}")
        ...
        if 'overall' in comparison:
            ...
            lines.append(f"Real dyads - Mean ICD: {c['mean_real']:.4f} (n={c['n_real']})")
            lines.append(f"Pseudo-dyads - Mean ICD: {c['mean_pseudo']:.4f} (n={c['n_pseudo']})")

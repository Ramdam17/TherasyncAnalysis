...
from src.visualization.config import (
    COLORS, ALPHA, FIGSIZE, FONTSIZE, LINEWIDTH,
    apply_plot_style, DPI
)
...
class AllianceICDStatsPlotter:
    ...
    def plot_icd_by_alliance(
        ...
        if split_by_dyad_type:
            ...
            for i, alliance in enumerate(alliance_order):
                for j, dtype in enumerate(['real', 'pseudo']):
                    ...
                    if len(subset) > 0:
                        ...
                        if plot_type == 'violin':
                            parts = ax.violinplot(
                                [subset], positions=[pos], widths=width * 0.9,
            ...
            legend_patches = [
                mpatches.Patch(color=self.DYAD_COLORS['real'], label='Real Dyads', alpha=ALPHA['high']),
                mpatches.Patch(color=self.DYAD_COLORS['pseudo'], label='Pseudo-Dyads', alpha=ALPHA['high'])
            ]
            ax.legend(handles=legend_patches, loc='upper right', fontsize=FONTSIZE['legend'])
            ...
            for i, alliance in enumerate(alliance_order):
                ...
                if len(subset) > 0:
                    ...
                    if plot_type == 'violin':
                        parts = ax.violinplot(
                            [subset], positions=[positions[i]], widths=0.7,
                            showmeans=True, showextrema=True, showmedians=True
                        )
                        for pc in parts['bodies']:
                        ...
                        bp = ax.boxplot(
                            [subset], positions=[positions[i]], widths=0.6,
                            patch_artist=True
                        )
                        for patch in bp['boxes']:
        ...
        ax.set_title('ICD Distribution by Alliance State', fontsize=FONTSIZE['title'], fontweight='bold')
    ...
    def plot_icd_comparison_heatmap(
        ...
        ax.set_yticklabels([s.capitalize() for s in pivot.index], fontsize=FONTSIZE['label'])
            ...
            for j in range(len(pivot.columns)):
                ...
                if not np.isnan(value):
                    ...
                    ax.text(j, i, f'{value:.3f}', ha='center', va='center', 
                           color=text_color, fontsize=FONTSIZE['label'], fontweight='bold')
        
        plt.colorbar(im, ax=ax, label='Mean ICD')
        ax.set_title('Mean ICD by Alliance State and Dyad Type', 
    ...
    def plot_alliance_distribution(
        ...
        colors = [self.ALLIANCE_COLORS.get(s.capitalize(), COLORS['gray']) for s in counts.index]
        ...
        axes[0].set_title('Alliance State Distribution', fontsize=FONTSIZE['title'], fontweight='bold')
        ...
        bars1 = axes[1].bar(x - width/2, cross_tab.get('real', [0]*4), width, 
                            label='Real Dyads', color=self.DYAD_COLORS['real'])
        bars2 = axes[1].bar(x + width/2, cross_tab.get('pseudo', [0]*4), width,
                            label='Pseudo-Dyads', color=self.DYAD_COLORS['pseudo'])
        
        axes[1].set_xlabel('Alliance State', fontsize=FONTSIZE['label'])
        axes[1].set_ylabel('Count (epochs)', fontsize=FONTSIZE['label'])
        axes[1].set_title('Epoch Count by Alliance and Dyad Type', 
                         fontsize=FONTSIZE['title'], fontweight='bold')
        axes[1].set_xticks(x)
    ...
    def plot_summary_dashboard(
        ...
        for i, alliance in enumerate(alliance_order):
            ...
            if len(subset) > 0:
                ...
                parts = ax1.violinplot([subset], positions=[i], widths=0.7,
        ...
        for i, alliance in enumerate(alliance_order):
            for j, dtype in enumerate(['real', 'pseudo']):
                subset = data[(data['alliance_label'] == alliance) & 
                             (data['dyad_type'] == dtype)]['icd']
                if len(subset) > 0:
                    pos = i + (j - 0.5) * 0.3
                    bp = ax2.boxplot([subset], positions=[pos], widths=0.25, patch_artist=True)
                    for patch in bp['boxes']:
                        patch.set_facecolor(self.DYAD_COLORS[dtype])
                        patch.set_alpha(ALPHA['high'])
        ...
        # Legend
        legend_patches = [
            mpatches.Patch(color=self.DYAD_COLORS['real'], label='Real', alpha=ALPHA['high']),
            mpatches.Patch(color=self.DYAD_COLORS['pseudo'], label='Pseudo', alpha=ALPHA['high'])
        ]
        ax2.legend(handles=legend_patches, loc='upper right', fontsize=FONTSIZE['small'])
        ...
        ax4.pie(counts.values, labels=labels, colors=colors, autopct='%1.1f%%', startangle=90)
        ...
        fig.suptitle('Alliance-ICD Analysis Dashboard', fontsize=FONTSIZE['title'] + 2, 
    ...
    def plot_icd_distributions_by_alliance(
        ...
        for i, alliance in enumerate(alliance_order):
            ...
            if len(real_data) > 0:
                ax.hist(
                    real_data, bins=bin_edges, 
                    alpha=0.6, label=f'Real (n={len(real_data):,})',
                    color=self.DYAD_COLORS['real'], density=True,
                    edgecolor='white', linewidth=0.5
                )
            
            if len(pseudo_data) > 0:
                ax.hist(
                    pseudo_data, bins=bin_edges,
                    alpha=0.6, label=f'Pseudo (n={len(pseudo_data):,})',
                    color=self.DYAD_COLORS['pseudo'], density=True,
                    edgecolor='white', linewidth=0.5
                )
            ...
            if len(real_data) > 0:
                ax.axvline(
                    real_data.mean(), color=self.DYAD_COLORS['real'],
            ...
            if len(pseudo_data) > 0:
                ax.axvline(
                    pseudo_data.mean(), color=self.DYAD_COLORS['pseudo'],
                    linestyle='--', linewidth=2, alpha=0.9,

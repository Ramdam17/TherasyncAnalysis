...
class MOIEpocher:
    ...
    def add_epoch_columns(
        ...
        logger.info(f"Adding epoch columns for {len(df)} annotations (duration: {duration:.1f}s)")
        ...
        for method in ['fixed', 'nsplit', 'sliding']:
            try:
                ...
                if method == 'fixed':
                    epoch_lists = self._assign_fixed_epochs(start_times, end_times, duration, task_config)
                elif method == 'nsplit':
                    epoch_lists = self._assign_nsplit_epochs(start_times, end_times, duration, task_config)
                elif method == 'sliding':
                    epoch_lists = self._assign_sliding_epochs(start_times, end_times, duration, task_config)
                else:
                ...
                if all_epoch_ids:
                    logger.debug(f"Assigned {method} epochs: range [{min(all_epoch_ids)}, {max(all_epoch_ids)}], "

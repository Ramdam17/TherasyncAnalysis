...
class MOIVisualizer:
    ...
    def plot_timeseries(
        ...
        # Row 0: Positive
        self._plot_binary_line(axes[0, 0], epochs, alliance_states, target=1, 
                              title='Alliance: Positive', color=self.colors['positive'])
        self._plot_binary_line(axes[0, 1], epochs, emotion_states, target=1,
                              title='Emotion: Positive', color=self.colors['positive'])
        
        # Row 1: Negative
        self._plot_binary_line(axes[1, 0], epochs, alliance_states, target=-1,
                              title='Alliance: Negative', color=self.colors['negative'])
        self._plot_binary_line(axes[1, 1], epochs, emotion_states, target=-1,
                              title='Emotion: Negative', color=self.colors['negative'])
        
        # Row 2: Mixed (Split)
        self._plot_binary_line(axes[2, 0], epochs, alliance_states, target=2,
                              title='Alliance: Mixed (Split)', color=self.colors['mixed'])
        self._plot_binary_line(axes[2, 1], epochs, emotion_states, target=2,
                              title='Emotion: Mixed (Split)', color=self.colors['mixed'])
        ...
        # Save figure
        output_file = output_dir / f"sub-{group_id}_ses-{session_id}_method-{method}_desc-alliance_timeseries.png"
    ...
    def _plot_binary_line(
        ...
        ax.plot(epochs, y, color=color, linewidth=2, marker='o', markersize=3, alpha=0.7)
    ...
    def plot_distribution(
        ...
        # Save figure
        output_file = output_dir / f"sub-{group_id}_ses-{session_id}_method-{method}_desc-alliance_distribution.png"
        plt.savefig(output_file, dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def visualize_session(
        ...
        for method in ['fixed', 'nsplit', 'sliding']:
            ...
            # Distribution
            dist_file = self.plot_distribution(df, group_id, session_id, method, output_dir)
            results['distribution'].append(dist_file)
        
        logger.info(f"✓ Created {len(results['timeseries']) + len(results['distribution'])} visualizations")

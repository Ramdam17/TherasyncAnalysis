...
class BIDSUtils:
    ...
    def parse_subject_id(self, subject_id: str) -> Dict[str, str]:
        ...
        if not subject_id.startswith('f') or 'p' not in subject_id:
            raise BIDSError(f"Invalid subject ID format: {subject_id}. Expected format: fXXpYY")
    ...
    def create_dataset_description(self, 
                                 ...
                                 version: str = "1.0.0",
                                 description: str = "Preprocessed physiological data from family therapy sessions") -> None:
        """
    ...
    def find_source_files(self,
                         ...
                         task: str,
                         signal_type: str,
                         source_root: str = "data") -> Dict[str, Path]:
        """
        ...
        if not result:
            raise BIDSError(f"No source files found for {subject} {session} {task} {signal_type}")
...
def create_bids_filename(subject: str, session: str, task: str, 
                        datatype: str, suffix: str, extension: str = ".tsv") -> str:
    """
    ...
    return bids_utils.create_bids_filename(subject, session, task, datatype, suffix, extension)

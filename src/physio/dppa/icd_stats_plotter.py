...
class ICDStatsPlotter:
    ...
    def load_icd_data(
        ...
        logger.info(f"Loaded: {n_real} real dyads, {n_pseudo} pseudo dyads, {n_epochs} epochs")
    ...
    def compute_participant_level_statistics(
        self,
        df: pd.DataFrame
    ) -> Dict:
        """
        ...
        """
        logger.info("Computing participant-level statistics (corrected for non-independence)")
        ...
        participant_real_means = {p: np.mean(vals) for p, vals in participant_real_icds.items()}
        participant_pseudo_means = {p: np.mean(vals) for p, vals in participant_pseudo_icds.items()}
        
        # Get participants with both real and pseudo data (for paired test)
        common_participants = set(participant_real_means.keys()) & set(participant_pseudo_means.keys())
        
        real_values = np.array([participant_real_means[p] for p in common_participants])
        pseudo_values = np.array([participant_pseudo_means[p] for p in common_participants])
        ...
        cohens_d_paired = np.mean(diff) / np.std(diff, ddof=1) if np.std(diff, ddof=1) > 0 else 0
    ...
    def compute_mixed_model_statistics(
        self,
        df: pd.DataFrame
    ) -> Dict:
        """
        ...
        try:
            ...
            re_variance = result.cov_re.iloc[0, 0] if hasattr(result, 'cov_re') else np.nan
            
            # Compute ICC (intraclass correlation)
            residual_var = result.scale
            icc = re_variance / (re_variance + residual_var) if (re_variance + residual_var) > 0 else 0
    ...
    def generate_corrected_statistics_report(
        ...
        report_lines = [
            ...
            "1. NAIVE ANALYSIS (treating dyads as independent)",
            "-" * 70,
            f"   ⚠️  WARNING: This analysis ignores non-independence!",
            ...
            "-" * 70,
            f"   ✓ Each participant's mean ICD computed, then compared",
        ...
        if naive_sig and not corrected_sig:
            report_lines.append("   ⚠️  CAUTION: Significance DISAPPEARS after correction!")
            report_lines.append("      The naive analysis may have inflated significance due to pseudo-replication.")
        elif naive_sig and corrected_sig:
            report_lines.append("   ✓ Effect remains significant after correction.")
            report_lines.append(f"      However, note the difference in effect size estimates.")
    ...
    def plot_real_vs_pseudo_distribution(
        ...
        # Violin plot
        vp = ax_main.violinplot(data, positions=positions, showextrema=False, widths=0.8)
        ...
        for i, (vals, color) in enumerate(zip(data, colors)):
            ...
            ax_main.scatter(
                positions[i] + jitter, vals_sample,
                c=color, alpha=0.3, s=MARKERSIZE['small'],
        ...
        for i, vals in enumerate(data):
            ax_main.scatter(
                [positions[i]], [np.mean(vals)],
                marker='D', s=100, c='white',
                edgecolors=colors[i], linewidths=2, zorder=10
            )
        ...
        ax_main.plot([0, 0, 1, 1], [bracket_y, bracket_y * 1.02, bracket_y * 1.02, bracket_y],
                     color=COLORS['dark_gray'], linewidth=LINEWIDTH['medium'])
        ax_main.text(0.5, bracket_y * 1.04, sig_symbol,
                     ha='center', va='bottom', fontsize=FONTSIZE['subtitle'],
                     fontweight='bold', color=COLORS['good'] if p_value < 0.05 else COLORS['poor'])
        
        # Formatting
        ax_main.set_xticks(positions)
        ax_main.set_xticklabels(labels, fontsize=FONTSIZE['label'], fontweight='bold')
        ax_main.set_ylabel('Inter-Centroid Distance (ms)', fontsize=FONTSIZE['label'], fontweight='bold')
        ax_main.set_title(
        ...
        # Legend
        legend_elements = [
            mpatches.Patch(facecolor=COLORS['real_dyad'], alpha=ALPHA['high'],
                          label=f"Real (n={metadata['n_real_dyads']} dyads)"),
            mpatches.Patch(facecolor=COLORS['pseudo_dyad'], alpha=ALPHA['high'],
                          label=f"Pseudo (n={metadata['n_pseudo_dyads']} dyads)"),
        ]
        ax_main.legend(handles=legend_elements, loc='upper right', fontsize=FONTSIZE['legend'])
        ...
        if 'corrected' in statistics and statistics['corrected'] is not None:
            ...
            if naive_p < 0.05 and corrected_p >= 0.05:
                stats_text += f"⚠️  Significance DISAPPEARS after correction for pseudo-replication!"
            elif corrected_p < 0.05:
                stats_text += f"✓ Effect remains significant after correction."
            else:
                stats_text += f"No significant difference between real and pseudo dyads."
        else:
            stats_text = (
    ...
    def plot_epoch_evolution(
        ...
        agg_df = df.groupby(['epoch_id', 'dyad_type'])['icd_value'].agg(
            ['mean', 'std', 'count']
        ).reset_index()
        ...
        for dyad_type, color, label in [
            ...
            ax.plot(epochs, means, color=color, linewidth=LINEWIDTH['medium'],
        ...
        ax.text(0.02, 0.98, 'Shaded area: ± SEM',
                transform=ax.transAxes, fontsize=FONTSIZE['small'],
                verticalalignment='top', color=COLORS['dark_gray'])
    ...
    def plot_dyad_heatmap(
        ...
        ax.set_xticklabels(subjects, rotation=45, ha='right', fontsize=FONTSIZE['small'])
        ax.set_yticklabels(subjects, fontsize=FONTSIZE['small'])
        
        ax.set_title('Inter-Centroid Distance Matrix\n(Mean across epochs)',
                     fontsize=FONTSIZE['title'], fontweight='bold')
    ...
    def plot_intra_family_evolution(
        ...
        # Aggregate by epoch
        agg_df = df.groupby('epoch_id')['icd_value'].agg(
            ['mean', 'std', 'count']
        ).reset_index()
        agg_df['sem'] = agg_df['std'] / np.sqrt(agg_df['count'])
        ...
        # Line plot with confidence band
        ax.plot(epochs, means, color=COLORS['real_dyad'], 
                linewidth=LINEWIDTH['medium'], label='Real Dyads',
                alpha=ALPHA['line'], marker='o', markersize=MARKERSIZE['small'])
        ax.fill_between(epochs, means - sems, means + sems,
                       color=COLORS['real_dyad'], alpha=ALPHA['fill'])
        
        # Add trend line
        z = np.polyfit(epochs, means, 1)
        p = np.poly1d(z)
        ax.plot(epochs, p(epochs), '--', color=COLORS['dark_gray'], 
                alpha=ALPHA['medium'], linewidth=LINEWIDTH['thin'],
    ...
    def plot_intra_family_distribution(
        ...
        # Histogram
        ax.hist(icd_values, bins=50, color=COLORS['real_dyad'], 
                alpha=ALPHA['high'], edgecolor='white', linewidth=0.5)
        
        # Add mean and median lines
        mean_val = np.mean(icd_values)
        median_val = np.median(icd_values)
        ax.axvline(mean_val, color=COLORS['dark_gray'], 
                   linewidth=LINEWIDTH['thick'], linestyle='--')
        ax.axvline(median_val, color=COLORS['dark_gray'], 
                   linewidth=LINEWIDTH['medium'], linestyle=':')

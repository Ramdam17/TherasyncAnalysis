...
class PoincareCalculator:
    ...
    def compute_centroids_for_file(
        self,
        rr_file: Path,
        method: str
    ) -> pd.DataFrame:
        ...
        df[epoch_col] = df[epoch_col].apply(lambda x: ast.literal_eval(x) if pd.notna(x) else [])
        ...
        result_df = result_df[['epoch_id', 'centroid_x', 'centroid_y', 
                               'sd1', 'sd2', 'sd_ratio', 'n_intervals']]
        
        logger.debug(f"Computed {len(results)} centroids for {rr_file.name} (method: {method})")
        return result_df

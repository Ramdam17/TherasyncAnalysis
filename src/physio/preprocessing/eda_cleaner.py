...
class EDACleaner:
    ...
    def clean_signal(
        ...
        try:
            ...
            if 'SCR_RecoveryTime' in signals.columns:
                processed_signals['SCR_RecoveryTime'] = signals['SCR_RecoveryTime'].values
            else:
    ...
    def get_scr_metadata(
        ...
        mean_scr_amplitude = float(scr_amplitudes.mean()) if len(scr_amplitudes) > 0 else 0.0
        max_scr_amplitude = float(scr_amplitudes.max()) if len(scr_amplitudes) > 0 else 0.0
        
        # SCR timing statistics
        scr_rise_times = processed_signals.loc[
            processed_signals['SCR_Peaks'] == 1, 'SCR_RiseTime'
        ]
        mean_rise_time = float(scr_rise_times.mean()) if len(scr_rise_times) > 0 else 0.0
        
        scr_recovery_times = processed_signals.loc[
            processed_signals['SCR_Peaks'] == 1, 'SCR_RecoveryTime'
        ]
        mean_recovery_time = float(scr_recovery_times.mean()) if len(scr_recovery_times) > 0 else 0.0
    ...
    def compute_scr_features(self, processed_signals: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(scr_indices) == 0:
            ...
            return pd.DataFrame(columns=[
                'scr_index', 'scr_time', 'scr_amplitude',

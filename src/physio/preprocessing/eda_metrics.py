...
class EDAMetricsExtractor:
    ...
    def extract_eda_metrics(
        ...
        phasic_metrics = self._extract_phasic_metrics(processed_signals, duration_seconds)
    ...
    def _validate_input(self, processed_signals: pd.DataFrame) -> None:
        ...
        """
        required_columns = [
            'EDA_Tonic', 'EDA_Phasic', 'SCR_Peaks',
    ...
    def _extract_scr_metrics(
        ...
        # SCR rate (per minute)
        duration_minutes = duration_seconds / 60
        metrics['SCR_Peaks_Rate'] = (num_scrs / duration_minutes) if duration_minutes > 0 else 0.0
    ...
    def _extract_tonic_metrics(self, processed_signals: pd.DataFrame) -> Dict[str, float]:
        """
    ...
    def _extract_phasic_metrics(
        ...
        duration_minutes = duration_seconds / 60
        metrics['EDA_Phasic_Rate'] = (positive_phasic / duration_minutes) if duration_minutes > 0 else 0.0
    ...
    def extract_multiple_moments(
        ...
        for moment_name, processed_signals in moment_data.items():
            try:
                metrics = self.extract_eda_metrics(processed_signals, moment=moment_name)
        ...
        logger.info(f"Successfully extracted metrics for {len(combined_metrics)} moments")

...
from ..config import (
    ...
    apply_plot_style, get_moment_color, get_moment_label,
    get_modality_color, get_metric_label, format_duration
)
...
def _plot_poincare_panel(ax: plt.Axes, bvp_data: Dict, data: Dict) -> None:
    ...
    ax.set_title('Poincaré Plot', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    ax.axis('off')  # Turn off the main axes - we'll use insets
    
    signals = bvp_data.get('signals', {})
    if not signals:
        ax.text(0.5, 0.5, 'No BVP signals', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    for idx, (moment_name, rr) in enumerate(sorted(moment_data.items())):
        ...
        # Scatter plot
        inset_ax.scatter(rr_n, rr_n1, c=color, alpha=0.5, 
                        s=MARKERSIZE['small'], edgecolors='none')
        
        # Identity line
        inset_ax.plot([rr_min, rr_max], [rr_min, rr_max],
                     '--', color=COLORS['gray'], alpha=0.5, linewidth=LINEWIDTH['thin'])
        ...
        ellipse = Ellipse(
            (rr_mean, rr_mean), width=2*sd2, height=2*sd1, angle=45,
            facecolor=color, alpha=0.15, edgecolor=color, linewidth=LINEWIDTH['medium']
        )
        ...
        inset_ax.set_title(f'{label}\nSD1={sd1:.0f}ms  SD2={sd2:.0f}ms', 
                          fontsize=FONTSIZE['small'], fontweight='bold', color=color, pad=3)
    ...
    ax.text(0.5, -0.02, 
            ...
            transform=ax.transAxes, fontsize=FONTSIZE['small'], color=COLORS['dark_gray'],
...
def _plot_autonomic_balance_panel(ax: plt.Axes, metrics: pd.DataFrame) -> None:
    ...
    ax.set_title('Autonomic Balance', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    
    if metrics is None or metrics.empty:
        ax.text(0.5, 0.5, 'No HRV metrics', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    if len(sdnn_values) == 0 or len(rmssd_values) == 0:
        ax.text(0.5, 0.5, 'Missing SDNN/RMSSD data', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    
    # Bar plot
    bars1 = ax.bar(x - width/2, sdnn_values, width, label='SDNN', 
                   color=COLORS['lf'], alpha=ALPHA['high'], edgecolor='white', linewidth=1)
    bars2 = ax.bar(x + width/2, rmssd_values, width, label='RMSSD', 
                   color=COLORS['hf'], alpha=ALPHA['high'], edgecolor='white', linewidth=1)
    
    # Value labels on bars
    for bar, val in zip(bars1, sdnn_values):
        if not np.isnan(val):
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 5,
                    f'{val:.0f}', ha='center', va='bottom', fontsize=FONTSIZE['small'],
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([get_moment_label(m) for m in moments], fontsize=FONTSIZE['tick'])
    ...
    ax.text(0.5, -0.12, 'SDNN: Overall variability  •  RMSSD: Parasympathetic activity',
            transform=ax.transAxes, fontsize=FONTSIZE['small'], color=COLORS['dark_gray'],
            ha='center', va='top', fontweight='bold')
...
def _plot_frequency_domain_panel(ax: plt.Axes, metrics: pd.DataFrame) -> None:
    ...
    ax.set_title('Frequency Domain Analysis', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    
    if metrics is None or metrics.empty:
        ax.text(0.5, 0.5, 'No frequency domain data', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    if not (has_lf or has_hf):
        ax.text(0.5, 0.5, 'No LF/HF data available\n(Frequency analysis requires longer recordings)',
                ha='center', va='center', transform=ax.transAxes, 
                fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    lf_values = [safe_float(v) for v in metrics['HRV_LF'].values] if has_lf else [np.nan] * len(moments)
    hf_values = [safe_float(v) for v in metrics['HRV_HF'].values] if has_hf else [np.nan] * len(moments)
    lfhf_values = [safe_float(v) for v in metrics['HRV_LFHF'].values] if has_lfhf else [np.nan] * len(moments)
    
    # Create grouped bar chart - only plot non-NaN values
    bars1 = ax.bar(x - width/2, [v if not np.isnan(v) else 0 for v in lf_values], width, 
                   label='LF (0.04-0.15 Hz)', 
                   color=COLORS['lf'], alpha=ALPHA['high'], edgecolor='white', linewidth=1)
    bars2 = ax.bar(x + width/2, [v if not np.isnan(v) else 0 for v in hf_values], width, 
    ...
    for i, (bar1, bar2, lf, hf) in enumerate(zip(bars1, bars2, lf_values, hf_values)):
        if not np.isnan(lf) and lf > 0:
            ax.text(bar1.get_x() + bar1.get_width()/2, bar1.get_height() + 0.002,
                    f'{lf:.3f}', ha='center', va='bottom', fontsize=FONTSIZE['small'],
                    fontweight='bold', color=COLORS['lf'])
        elif np.isnan(lf) or lf == 0:
            ax.text(bar1.get_x() + bar1.get_width()/2, 0.001,
                    'N/A', ha='center', va='bottom', fontsize=FONTSIZE['small'],
                    color=COLORS['gray'], style='italic')
        
        if not np.isnan(hf) and hf > 0:
            ax.text(bar2.get_x() + bar2.get_width()/2, bar2.get_height() + 0.002,
                    f'{hf:.3f}', ha='center', va='bottom', fontsize=FONTSIZE['small'],
                    fontweight='bold', color=COLORS['hf'])
    
    # Secondary y-axis for LF/HF ratio - properly centered on each moment
    valid_lfhf = [(i, v) for i, v in enumerate(lfhf_values) if not np.isnan(v) and v > 0]
    if valid_lfhf:
        ax2 = ax.twinx()
        ...
        ax2.plot(x_ratio, y_ratio, 's-', color=COLORS['scr'], 
                linewidth=LINEWIDTH['thick'], markersize=MARKERSIZE['large'],
                label='LF/HF Ratio', markeredgecolor='white', markeredgewidth=1)
        
        # Add ratio values next to markers
        for xi, yi in zip(x_ratio, y_ratio):
            ax2.annotate(f'{yi:.2f}', xy=(xi, yi), xytext=(5, 5), 
                        textcoords='offset points', fontsize=FONTSIZE['small'],
                        fontweight='bold', color=COLORS['scr'])
...
def _plot_arousal_profile_panel(ax: plt.Axes, eda_data: Dict) -> None:
    ...
    """
    ax.set_title('Arousal Profile', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    
    metrics = eda_data.get('metrics')
    if metrics is None or metrics.empty:
        ax.text(0.5, 0.5, 'No EDA metrics', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    tonic_values = metrics['EDA_Tonic_Mean'].values if 'EDA_Tonic_Mean' in metrics.columns else []
    scr_rate_values = metrics['SCR_Peaks_Rate'].values if 'SCR_Peaks_Rate' in metrics.columns else []
    
    if len(tonic_values) == 0:
        ax.text(0.5, 0.5, 'No tonic data', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    for i, moment in enumerate(moments):
        ...
        ax.bar(x[i] - width/2, tonic_values[i], width, 
               color=moment_color, alpha=ALPHA['high'], edgecolor='white', linewidth=1,
               label='Tonic EDA (µS)' if i == 0 else '')
        # Value label
        ax.text(x[i] - width/2, tonic_values[i] + 0.02, f'{tonic_values[i]:.2f}', 
                ha='center', va='bottom', fontsize=FONTSIZE['small'], fontweight='bold')
    ...
    if len(scr_rate_values) > 0:
        ...
        for i, moment in enumerate(moments):
            moment_color = get_moment_color(moment)
            ax2.bar(x[i] + width/2, scr_rate_values[i], width,
                   color=moment_color, alpha=ALPHA['medium'], edgecolor=moment_color, 
                   linewidth=1.5, hatch='///',
                   label='SCR Rate (/min)' if i == 0 else '')
            # Value label
            ax2.text(x[i] + width/2, scr_rate_values[i] + 0.5, f'{scr_rate_values[i]:.1f}', 
                    ha='center', va='bottom', fontsize=FONTSIZE['small'], fontweight='bold')
    ...
    if len(scr_rate_values) > 0:
        ax2.legend(loc='upper left', bbox_to_anchor=(1.15, 0.85), fontsize=FONTSIZE['legend'])
    
    # Interpretation note
    ax.text(0.5, -0.12, 'Solid: Tonic (baseline arousal)  •  Hatched: SCR Rate (reactivity)',
            transform=ax.transAxes, fontsize=FONTSIZE['small'], color=COLORS['dark_gray'],
            ha='center', va='top', fontweight='bold')
...
def _plot_scr_distribution_panel(ax: plt.Axes, eda_data: Dict) -> None:
    ...
    ax.set_title('SCR Amplitude Distribution', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    
    events = eda_data.get('events', {})
    if not events:
        ax.text(0.5, 0.5, 'No SCR events', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    if not moment_amplitudes:
        ax.text(0.5, 0.5, 'No SCR amplitudes', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    for i, (patch, moment) in enumerate(zip(bp['boxes'], moments)):
        ...
        ax.text(positions[i], ax.get_ylim()[1] * 0.95 if ax.get_ylim()[1] > 0 else median * 1.5,
                f'n={n}', ha='center', va='bottom', fontsize=FONTSIZE['small'],
...
def _plot_scr_timeline_panel(ax: plt.Axes, eda_data: Dict, data: Dict) -> None:
    ...
    ax.set_title('SCR Events Timeline', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    ...
    for moment_name, moment_events in events.items():
        ...
        if 'onset' not in moment_events.columns or 'amplitude' not in moment_events.columns:
            continue
        ...
        markerline, stemlines, baseline = ax.stem(times, amps, linefmt=color, 
                                                   markerfmt='o', basefmt=' ')
        ...
        # Add moment label
        mid = (moment_boundaries[moment_name]['start'] + moment_boundaries[moment_name]['end']) / 2
        ax.text(mid, ax.get_ylim()[1] * 0.95 if ax.get_ylim()[1] > 0 else 0.5,
                get_moment_label(moment_name),
                ha='center', va='top', fontsize=FONTSIZE['annotation'],
...
def _plot_temp_timeline_panel(ax: plt.Axes, temp_data: Dict) -> None:
    ...
    ax.set_title('Temperature Timeline', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    ...
    for moment_name in sorted(signals.keys()):
        ...
        color = get_moment_color(moment_name)
        label = get_moment_label(moment_name)
        ax.plot(time, temp, color=color, linewidth=LINEWIDTH['medium'],
                label=label, alpha=ALPHA['line'])
...
def _plot_temp_metrics_panel(ax: plt.Axes, temp_data: Dict) -> None:
    ...
    ax.set_title('Temperature Summary', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    
    metrics = temp_data.get('metrics')
    if metrics is None or metrics.empty:
        ax.text(0.5, 0.5, 'No temperature metrics', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    
    moments = metrics['moment'].tolist() if 'moment' in metrics.columns else []
    if not moments:
        ax.text(0.5, 0.5, 'No moment data in metrics', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    if mean_col is None:
        ax.text(0.5, 0.5, f'No temp mean column found\nAvailable: {list(metrics.columns)[:5]}...', 
                ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    bars = ax.bar(x, means, width, yerr=sds, capsize=5, 
                  color=colors, alpha=ALPHA['high'], edgecolor='white', linewidth=1,
                  error_kw={'elinewidth': LINEWIDTH['normal'], 'capthick': LINEWIDTH['normal']})
    
    # Value labels
    for bar, mean, sd in zip(bars, means, sds):
        if not np.isnan(mean):
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + sd + 0.1,
                    f'{mean:.1f}°C', ha='center', va='bottom', fontsize=FONTSIZE['annotation'],
                    fontweight='bold')
...
def _plot_temp_eda_correlation_panel(ax: plt.Axes, temp_data: Dict, eda_data: Dict) -> None:
    """
    ...
    """
    ax.set_title('Temp-EDA Correlation', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    ...
    if not common_moments:
        ax.text(0.5, 0.5, 'No matching moments', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'], color=COLORS['gray'])
        return
    ...
    for moment_name in common_moments:
        ...
        temp_resampled = np.interp(
            ...
            np.linspace(0, 1, len(temp_vals)),
            temp_vals
        )
        eda_resampled = np.interp(
            np.linspace(0, 1, min_len),
            np.linspace(0, 1, len(eda_vals)),
            eda_vals
        )
    ...
    bars = ax.barh(x, correlations, color=colors, alpha=ALPHA['high'], 
    ...
    for bar, corr in zip(bars, correlations):
        ...
        if corr >= 0:
            ax.text(corr + 0.02, bar.get_y() + bar.get_height()/2,
                    f'r = {corr:.2f}', ha='left', va='center', 
                    fontsize=FONTSIZE['annotation'], fontweight='bold')
        else:
            ax.text(corr - 0.02, bar.get_y() + bar.get_height()/2,
                    f'r = {corr:.2f}', ha='right', va='center',
                    fontsize=FONTSIZE['annotation'], fontweight='bold')
    ...
    # Y-axis labels
    ax.set_yticks(x)
    ax.set_yticklabels([get_moment_label(m) for m in valid_moments], fontsize=FONTSIZE['tick'])
    ...
    ax.text(0.5, -0.15, 'Negative r: Stress → ↓Temp (vasoconstriction)  •  Positive r: Relaxation → ↑Temp',
            transform=ax.transAxes, fontsize=FONTSIZE['small'], color=COLORS['dark_gray'],
            ha='center', va='top', fontweight='bold')
...
def _plot_quality_overview_panel(ax: plt.Axes, data: Dict, fig: plt.Figure = None) -> None:
    """Plot mean quality score per modality and moment."""
    ax.set_title('Signal Quality by Modality', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    ...
    for modality in modalities:
        ...
        for moment_name, moment_signals in signals.items():
            ...
            quality_col = None
            for col in [f'{modality.upper()}_Quality', 'PPG_Quality', 'EDA_Quality', 
                        'HR_Quality', 'TEMP_Quality', 'quality']:
                if col in moment_signals.columns:
                    quality_col = col
                    break
    ...
    for i, moment in enumerate(moments):
        ...
        bars = ax.bar(x + offset, values, width * 0.9, label=get_moment_label(moment),
                      color=color, alpha=ALPHA['high'], edgecolor='white')
        handles.append(bars[0])
    
    # Add quality threshold lines
    line_acceptable = ax.axhline(0.7, color=COLORS['medium'], linestyle='--', 
                                  linewidth=LINEWIDTH['normal'], alpha=0.7)
    line_good = ax.axhline(0.9, color=COLORS['good'], linestyle='--', 
                            linewidth=LINEWIDTH['normal'], alpha=0.7)
    ...
    legend_labels = [get_moment_label(m) for m in moments] + ['Acceptable (70%)', 'Good (90%)']
    ax.legend(legend_handles, legend_labels, loc='upper left', bbox_to_anchor=(1.02, 1),
              fontsize=FONTSIZE['legend'], frameon=True, fancybox=True)
...
def _plot_data_coverage_panel(ax: plt.Axes, data: Dict, fig: plt.Figure = None) -> None:
    ...
    """
    ax.set_title('Data Quality Coverage', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    ...
    for modality in modalities:
        ...
        for moment_signals in signals.values():
            ...
            if quality_col:
                ...
                for col in [f'{modality.upper()}_Clean', f'{modality.upper()}_Raw', 
                            'PPG_Clean', 'EDA_Clean', 'TEMP_Clean']:
                    if col in moment_signals.columns:
                        valid_samples += moment_signals[col].notna().sum()
                        good_samples += moment_signals[col].notna().sum()
                        break
        
        coverage_good[modality] = good_samples / total_samples * 100 if total_samples > 0 else 0
        coverage_valid[modality] = valid_samples / total_samples * 100 if total_samples > 0 else 0
    ...
    bars1 = ax.bar(x - width/2, [coverage_valid[m] for m in modalities], width,
                   color=colors, alpha=ALPHA['medium'], edgecolor='white', linewidth=1)
    
    # Good quality samples (darker with hatch)
    bars2 = ax.bar(x + width/2, [coverage_good[m] for m in modalities], width,
                   color=colors, alpha=ALPHA['high'], edgecolor='white', linewidth=1,
                   hatch='///')
    ...
    legend_elements = [
        Patch(facecolor=COLORS['gray'], alpha=ALPHA['medium'], edgecolor='white', label='Valid'),
        Patch(facecolor=COLORS['gray'], alpha=ALPHA['high'], hatch='///', edgecolor='white', label='Good (≥70%)')
    ]
    ax.legend(handles=legend_elements, loc='lower left', fontsize=FONTSIZE['small'], 
              frameon=True, fancybox=True, framealpha=0.9)


def _plot_quality_distribution_panel(ax: plt.Axes, data: Dict, fig: plt.Figure = None) -> None:
    """
    ...
    """
    ax.set_title('Quality Score Distribution', fontsize=FONTSIZE['subtitle'], fontweight='bold', pad=10)
    ...
    for i, modality in enumerate(modalities):
        ...
        if modality_quality[modality] is not None and len(modality_quality[modality]) > 0:
            ...
            if std_val < 0.001:  # Constant values (like HR/TEMP with all 1.0)
                ...
                ax.barh(mean_val, 0.6, height=0.04, left=i - 0.3, color=color, 
                        alpha=ALPHA['high'], edgecolor='white', linewidth=1.5)
                # Add diamond marker
                ax.scatter(i, mean_val, s=100, color=color, marker='D', 
                          edgecolors='white', linewidths=2, zorder=5)
                # Add value annotation
                ax.text(i, mean_val + 0.06, f'{mean_val:.2f}', 
                        ha='center', va='bottom', fontsize=FONTSIZE['small'],
                        fontweight='bold', color=COLORS['text'])
                
                legend_handles.append(Patch(facecolor=color, alpha=ALPHA['high'], 
                                            edgecolor='white', label=f'{modality.upper()} (constant)'))
            else:
                # Draw boxplot for variable data
                bp = ax.boxplot([vals], positions=[i], widths=0.6, 
                ...
                legend_handles.append(Patch(facecolor=color, alpha=ALPHA['high'], 
                                            edgecolor='white', label=modality.upper()))
        else:
            # Draw placeholder bar with "N/A" label
            ax.bar(i, 0.5, width=0.6, color=color, alpha=0.2, edgecolor=color, 
                   linewidth=2, linestyle='--')
            ax.text(i, 0.55, 'N/A', ha='center', va='bottom', 
                    fontsize=FONTSIZE['annotation'], color=COLORS['gray'],
                    fontweight='bold', fontstyle='italic')
            
            legend_handles.append(Patch(facecolor=color, alpha=0.2, 
                                        edgecolor=color, linestyle='--', 
                                        label=f'{modality.upper()} (N/A)'))
    
    # Add quality threshold lines
    line_acceptable = ax.axhline(0.7, color=COLORS['medium'], linestyle='--', 
                                  linewidth=LINEWIDTH['medium'], alpha=0.8)
    line_good = ax.axhline(0.9, color=COLORS['good'], linestyle='--', 
                            linewidth=LINEWIDTH['medium'], alpha=0.8)
    ...
    legend_handles.extend([
        plt.Line2D([0], [0], color=COLORS['medium'], linestyle='--', 
                   linewidth=LINEWIDTH['medium'], label='Acceptable (70%)'),
        plt.Line2D([0], [0], color=COLORS['good'], linestyle='--', 
                   linewidth=LINEWIDTH['medium'], label='Good (90%)')
    ])

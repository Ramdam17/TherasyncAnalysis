...
def plot_temp_timeline(
    ...
    if not signals:
        ax.text(0.5, 0.5, 'No temperature signals available', ha='center', va='center',
                transform=ax.transAxes, fontsize=FONTSIZE['label'])
        if output_path:
            fig.savefig(output_path, dpi=300, bbox_inches='tight')
        if show:
    ...
    ax.axhspan(y_min, 32.0, color=COLORS['good'], alpha=0.1, 
               label='Cool (<32°C)', zorder=0)
    # Normal zone (32-35°C)
    ax.axhspan(32.0, 35.0, color=COLORS['medium'], alpha=0.1, 
               label='Normal (32-35°C)', zorder=0)
    # Warm zone (above 35°C)
    ax.axhspan(35.0, y_max, color=COLORS['poor'], alpha=0.1, 
               label='Warm (>35°C)', zorder=0)
    ...
    for idx, m in enumerate(moment_data):
        ...
        ax.plot(m['time'], m['temp'], color=color, linewidth=LINEWIDTH['signal'],
                alpha=ALPHA['line'], label=label)
        
        # Add moment separator (vertical line) - except for last
        if idx < len(moment_data) - 1:
            ax.axvline(m['end'] + 5, color=COLORS['dark_gray'], linestyle='--',
                       linewidth=LINEWIDTH['thin'], alpha=0.5, zorder=10)
    ...
    ax.set_title('Body Temperature Dynamics Timeline\n(with physiological zones)',
...
def plot_temp_metrics_comparison(
    ...
    if not temp_data or 'signals' not in temp_data:
        ax1.text(0.5, 0.5, 'No temperature data available', ha='center', va='center',
    ...
    if not stats:
        ax1.text(0.5, 0.5, 'Insufficient temperature data', ha='center', va='center',
                 transform=ax1.transAxes, fontsize=FONTSIZE['label'])
        if output_path:
            fig.savefig(output_path, dpi=300, bbox_inches='tight')
        if show:
    ...
    bars = ax1.bar(x, means, 0.6, color=colors, alpha=0.8,
                   edgecolor='white', linewidth=2)
    
    # Add error bars
    ax1.errorbar(x, means, yerr=stds, fmt='none', ecolor=COLORS['dark_gray'],
                 elinewidth=2, capsize=5, capthick=2)
    
    # Add min/max range markers
    for i, (moment, mean, min_val, max_val) in enumerate(zip(moments, means, mins, maxs)):
        ax1.scatter([i], [min_val], marker='v', color=colors[i], 
                    s=50, zorder=5, edgecolors='white', linewidths=1)
        ax1.scatter([i], [max_val], marker='^', color=colors[i], 
                    s=50, zorder=5, edgecolors='white', linewidths=1)
        
        # Value label on bar
        ax1.text(i, mean + max(stds) * 0.1, f'{mean:.1f}°C',
    ...
    bars2 = ax2.bar(x, slopes, 0.6, color=slope_colors, alpha=0.8,
                    edgecolor='white', linewidth=2)
    ...
    for i, (moment, slope) in enumerate(zip(moments, slopes)):
        ...
        ax2.text(i, slope + y_offset, f'{slope:.3f}',
                 ha='center', va=va, fontsize=FONTSIZE['annotation'],
                 fontweight='bold', color=slope_colors[i])
    ...
    ax2.set_xticks(x)
    ax2.set_xticklabels([get_moment_label(m) for m in moments], 
                        fontsize=FONTSIZE['tick'], fontweight='bold')
    ax2.grid(True, alpha=ALPHA['fill'], axis='y', linestyle='--')
    ax2.set_title('Temperature Trend by Moment\n(positive = warming, negative = cooling)',
                  fontsize=FONTSIZE['subtitle'], fontweight='bold')
    ...
    fig.suptitle(
        ...
        f'Subject {data.get("subject", "Unknown")}, Session {data.get("session", "Unknown")}',
        fontsize=FONTSIZE['title'], fontweight='bold', y=0.99)

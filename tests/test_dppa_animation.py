...
class TestEpochAnimator:
    ...
    def sample_rr_df(self):
        ...
        return pd.DataFrame({
            ...
            'is_valid': [1, 1, 1, 1, 1],
            'epoch_sliding_duration30s_step5s': ['[0]', '[0, 1]', '[1]', '[1, 2]', '[2]']
        })
    ...
    def test_load_rr_intervals_file_not_found(self, animator):
        ...
        with pytest.raises(FileNotFoundError):
            animator.load_rr_intervals('invalid_sub', 'ses-99', 'therapy', 'sliding_duration30s_step5s')
    ...
    def test_get_rr_for_epoch(self, animator, sample_rr_df):
        ...
        rr_epoch_0 = animator.get_rr_for_epoch(sample_rr_df, 0, 'sliding_duration30s_step5s')
        assert len(rr_epoch_0) == 2
        np.testing.assert_array_equal(rr_epoch_0, [750.0, 760.0])
        
        # Epoch 1: intervals at indices 1, 2, 3
        rr_epoch_1 = animator.get_rr_for_epoch(sample_rr_df, 1, 'sliding_duration30s_step5s')
        assert len(rr_epoch_1) == 3
        np.testing.assert_array_equal(rr_epoch_1, [760.0, 755.0, 765.0])
        
        # Epoch 2: intervals at indices 3, 4
        rr_epoch_2 = animator.get_rr_for_epoch(sample_rr_df, 2, 'sliding_duration30s_step5s')
    ...
    def test_compute_poincare_for_epoch(self, animator, sample_rr_df):
        ...
        result = animator.compute_poincare_for_epoch(sample_rr_df, 1, 'sliding_duration30s_step5s')
...
class TestPoincarePlotter:
    ...
    def test_calculate_ellipse_parameters(self, plotter, sample_poincare_data):
        ...
        params = plotter.calculate_ellipse_parameters(
            sample_poincare_data['rr_n'],
            sample_poincare_data['rr_n_plus_1']
        )
        ...
        assert params['centroid_x'] == pytest.approx(np.mean(sample_poincare_data['rr_n']))
        assert params['centroid_y'] == pytest.approx(np.mean(sample_poincare_data['rr_n_plus_1']))
    ...
    def test_annotate_metrics(self, plotter):
        ...
        plotter.annotate_metrics(
            ax, 123.4, 45.6, 78.9, 52.1, 81.3, "S1", "S2"
        )

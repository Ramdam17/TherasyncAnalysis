    ...
    def test_data_validation(self):
        ...
        with patch('src.physio.preprocessing.temp_loader.ConfigLoader') as mock_config:
            ...
            with self.assertRaises((ValueError, KeyError)):
                loader._validate_temp_data(invalid_data, valid_metadata, Path("test.tsv"))
...
class TestTEMPCleaner(unittest.TestCase):
    ...
    def _create_test_signal(self):
        ...
        temp = 32.0 + 0.3 * np.sin(2 * np.pi * 0.002 * t) + np.random.normal(0, 0.05, len(t))
...
class TestTEMPMetricsExtractor(unittest.TestCase):
    ...
    def _create_test_data(self):
        ...
        temp_values = 31.5 + 0.5 * (time_values / 60) + np.random.normal(0, 0.03, n_samples)
    ...
    def test_stability_metrics(self, mock_config):
        ...
        n_samples = 240
        time_values = np.arange(0, 60, 0.25)
        temp_values = 32.0 + np.random.normal(0, 0.01, n_samples)  # Very low variability
...
class TestTEMPPipelineIntegration(unittest.TestCase):
    ...
    def test_full_pipeline_execution(self, mock_writer_config, mock_metrics_config,
        ...
        # Step 2: Clean data
        cleaner = TEMPCleaner()
        cleaned_data, cleaning_metadata = cleaner.clean_signal(data, moment=self.test_moment)
